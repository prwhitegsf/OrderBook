...
void Evaluator::evaluate_order(order::BuyLimit o) const {
    ...
    {
        for (auto su : state_update_q_)
        {
            if (su.id == 0)
            {
                su = order::StateUpdate(o.id,order::OrderState::REJECTED);
                break;
            }
        }
 ...
 void Evaluator::evaluate_order(order::BuyMarket o) const {
    ...
    else
    {
        ...
        else // but if we reached limit, place the remaining qty on the book as a limit order
        {
            d_.bid = limit;
